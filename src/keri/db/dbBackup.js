...
class Databaser {
  ...
  setVal(db, key, value) {
    try {
      ...
      console.log('Value of key inside SETVAL IS ------------------->', dbi);
  ...
  getVal(db, key) {
    ...
    try {
      ...
      const data = this.txn.getBinary(dbi, key);
  ...
  putVals(db, key, vals) {
    try {
      ...
      for (const val in vals) {
        ...
        console.log('VALUE SUCCESSFULLY ADDED', vals[val].toString());
      }
  ...
  * getValsIter(db, key) {
  ...
  addVal(db, key, val) {
    const dups = this.getVals(db, key);
    ...
    try {
      ...
      if (!dups.includes(val)) {
        ...
        this.txn.putBinary(dbi, key, Buffer.from(val, 'binary'), { overwrite: false, keyIsBuffer: true });
      }
      this.txn.commit();
      dbi.close();
      // this.env.close();
  ...
  putIOVals(db, key, vals) {
    ...
    try {
      ...
      if (!dups.includes(vals)) {
        ...
        count_pad = pad(count, 6);
        count_pad = `${count_pad}.`;
        const arr = [Buffer.from(count_pad, 'binary'), Buffer.from(vals.toString(), 'binary')];
...
function dgkey(pre, dig) {
  ...
  if (dig) dig = Buffer.from(dig, 'binary');

  const dot_buf = Buffer.from('.', 'binary');
