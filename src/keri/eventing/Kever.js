...
const {
  Serials, Versionage, IcpLabels, Ilks,
} = require('../core/core');
...
class Kever {
  constructor(serder, sigers, estOnly = null, logger = null) {
    ...
    if (typeof (sith) === 'string') {
      ...
      if ((this.sith).length < 1 || (this.sith).length > (this.verfers).length) throw `Invalid sith = ${sith} for keys = ${this.verfers.qb64()}`;
    } else {
    ...
    this.sn = sn;
    console.log('Value of Serder is =======>');
    ...
    } else if (toad != 0) throw `Invalid toad = ${toad} for wits = ${wits}`;
  ...
  async update(serder, sigers) {
    ...
    if (pre != this.prefixer.qb64()) throw `Mismatch event aid prefix = ${pre} expecting = ${this.prefixer.qb64()}.`;
    ...
    if (ilk == Ilks.rot) {
      ...
      if (sn > this.sn + 1) // Out of order event
      {
        ...
        if (sn <= this.lastEst.sn) {
          ...
          const pdig = this.logger.getKeLast(key = snkey(pre = pre, sn = psn));
  ...
  verifySith(sigers, sith = null) {
    ...
    if (!(typeof (sith) === 'number')) throw `Unsupported type for sith =${sith}`;

    if (sigers.length < sith.length) return false;
 ...
 * @param {*} sigers  sigers is list of Siger instances
  ...
  logEvent(serder, sigers) {
    console.log('\n\n INSIDE LOG EVENTS :');
    const dgkey_ = dgkey(this.prefixer.qb64b(), this.diger.qb64b());
    const date = new Date();
  ...
  escrowEvent(serder, sigers, pre, sn) {
    ...
    const dgkey_ = dgkey(pre, serder.digb());
    const date = new Date();
    this.logger.putDts(dgkey_, Buffer.from(date.toISOString(), 'binary'));
    for (const k in sigers) {
